...
def export_inventory_pdf(request):
    ...
    # Add title
    elements.append(Paragraph('🖥️ PCBULACAN', title_style))
    elements.append(Paragraph('INVENTORY REPORT', heading_style))
    elements.append(Spacer(1, 0.2*inch))
    
    # Add report info
    report_date = timezone.now().strftime('%B %d, %Y - %I:%M %p')
    info_text = f'<b>Report Generated:</b> {report_date} | <b>Generated By:</b> {request.user.get_full_name()}'
    ...
    summary_data = [
        ...
        [str(total_products), str(in_stock_count), str(low_stock_count), str(out_of_stock_count)]
    ]
    ...
    summary_table.setStyle(TableStyle([
        ...
        ('GRID', (0, 0), (-1, -1), 1, colors.HexColor('#048400')),
    ...
    table_data = [['Product', 'Category', 'Price (₱)', 'Stock', 'Status', 'Value (₱)']]
    ...
    # Create table
    inventory_table = Table(table_data, colWidths=[2.5*inch, 1.5*inch, 1*inch, 0.8*inch, 1*inch, 1.2*inch])
    ...
    # Footer
    footer_text = '<i>This report is computer-generated and confidential. PCBulacan - Inventory Management System</i>'
...
def export_orders_pdf(request):
    """Export orders report as PDF using ReportLab"""
    from reportlab.lib import colors
    from reportlab.lib.pagesizes import letter, landscape
    from reportlab.lib.styles import getSampleStyleSheet, ParagraphStyle
    from reportlab.lib.units import inch
    ...
    summary_data = [
        ...
        [str(total_orders), str(pending_orders), str(processing_orders), str(shipped_orders), str(delivered_orders), str(cancelled_orders)]
    ]
    ...
    # Orders table
    table_data = [['Order #', 'Customer', 'Date', 'Total (₱)', 'Payment', 'Status']]
    ...
    for order in orders[:50]:  # Limit to 50 orders
        ...
        table_data.append([order_number, customer_name, date, total, payment, status])
    
    # Add total revenue row
    table_data.append(['', '', '', '', 'Total Revenue:', f'₱{total_revenue:,.2f}'])
    
    orders_table = Table(table_data, colWidths=[1.5*inch, 2*inch, 1*inch, 1.2*inch, 1.2*inch, 1*inch])
    orders_table.setStyle(TableStyle([
        ...
        ('FONTSIZE', (0, 1), (-1, -2), 9),
        ('ALIGN', (0, 1), (-1, -2), 'LEFT'),
        ('ALIGN', (3, 1), (3, -2), 'RIGHT'),  # Total column
    ...
    # Footer
    elements.append(Spacer(1, 0.3*inch))
    footer_text = '<i>This is a computer-generated document and is considered confidential.</i>'
    footer_style = ParagraphStyle('Footer', parent=styles['Normal'], fontSize=9, alignment=TA_CENTER, textColor=colors.HexColor('#64748b'))
    ...
    response['Content-Disposition'] = f'attachment; filename="orders_report_{timestamp}.pdf"'
...
def export_payment_history_pdf(request):
    """Export payment history report as PDF using ReportLab"""
    from reportlab.lib import colors
    from reportlab.lib.pagesizes import letter, landscape
    from reportlab.lib.styles import getSampleStyleSheet, ParagraphStyle
    from reportlab.lib.units import inch
    ...
    # Get all completed orders with payment info
    orders = Order.objects.all().select_related('user').order_by('-created_at')
    
    # Statistics
    total_payments = orders.count()
    ...
    # Add title
    elements.append(Paragraph('🖥️ PCBULACAN', title_style))
    elements.append(Paragraph('PAYMENT HISTORY REPORT', heading_style))
    elements.append(Spacer(1, 0.2*inch))
    ...
    summary_data = [
        ['Summary Statistics'],
        ['Total Payments', 'Completed', 'Pending', 'Failed/Cancelled'],
        [str(total_payments), str(completed_payments), str(pending_payments), str(failed_payments)]
    ]
    ...
    table_data = [['Date', 'Order #', 'Customer', 'Payment Method', 'Amount (₱)', 'Status']]
    ...
    payment_table = Table(table_data, colWidths=[1*inch, 1.5*inch, 2*inch, 1.5*inch, 1.2*inch, 1*inch])
    payment_table.setStyle(TableStyle([
        ...
        ('FONTSIZE', (0, 1), (-1, -2), 9),
        ('ALIGN', (0, 1), (-1, -2), 'LEFT'),
        ('ALIGN', (4, 1), (4, -2), 'RIGHT'),  # Amount column
        ('VALIGN', (0, 1), (-1, -2), 'MIDDLE'),
        ('TOPPADDING', (0, 1), (-1, -2), 8),
        ('BOTTOMPADDING', (0, 1), (-1, -2), 8),
    ...
    response = HttpResponse(buffer, content_type='application/pdf')
    timestamp = timezone.now().strftime('%Y%m%d_%H%M%S')
    response['Content-Disposition'] = f'attachment; filename="payment_history_{timestamp}.pdf"'
...
def export_users_pdf(request):
    """Export users report as PDF using ReportLab"""
    from reportlab.lib import colors
    from reportlab.lib.pagesizes import letter, landscape
    from reportlab.lib.styles import getSampleStyleSheet, ParagraphStyle
    from reportlab.lib.units import inch
    ...
    # Add title
    elements.append(Paragraph('🖥️ PCBULACAN', title_style))
    elements.append(Paragraph('USER ACCOUNTS REPORT', heading_style))
    elements.append(Spacer(1, 0.2*inch))
    ...
    summary_data = [
        ['Summary Statistics'],
        ['Total Users', 'Active Users', 'Staff Members'],
        [str(total_users), str(active_users), str(staff_users)]
    ]
    
    summary_table = Table(summary_data, colWidths=[3*inch]*3)
    summary_table.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#048400')),
        ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
        ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
    ...
    # Users table
    table_data = [['Name', 'Email', 'Phone', 'Date Joined', 'Status', 'Role']]
    ...
    for user in users:
        ...
        table_data.append([name, email, phone, date_joined, status, role])
    
    users_table = Table(table_data, colWidths=[1.8*inch, 2.2*inch, 1.2*inch, 1*inch, 0.8*inch, 0.9*inch])
    users_table.setStyle(TableStyle([
        ...
        ('FONTSIZE', (0, 1), (-1, -1), 9),
        ('ALIGN', (0, 1), (-1, -1), 'LEFT'),
        ('ALIGN', (4, 1), (5, -1), 'CENTER'),  # Status and Role columns
    ...
    response = HttpResponse(buffer, content_type='application/pdf')
    timestamp = timezone.now().strftime('%Y%m%d_%H%M%S')
    response['Content-Disposition'] = f'attachment; filename="users_report_{timestamp}.pdf"'
    
    return response


@staff_member_required
def export_sales_analytics_pdf(request):
    """Export sales analytics report as PDF using ReportLab"""
    from reportlab.lib import colors
    from reportlab.lib.pagesizes import letter, landscape
    from reportlab.lib.styles import getSampleStyleSheet, ParagraphStyle
    from reportlab.lib.units import inch
    ...
    bank_count = orders.filter(payment_method='bank_transfer').count()
    ...
    # Add title
    elements.append(Paragraph('🖥️ PCBULACAN', title_style))
    elements.append(Paragraph('SALES ANALYTICS REPORT', heading_style))
    elements.append(Spacer(1, 0.2*inch))
    ...
    metrics_data = [
        ...
        [f'₱{total_revenue:,.2f}', str(total_orders), f'₱{avg_order_value:,.2f}']
    ]
    
    metrics_table = Table(metrics_data, colWidths=[3*inch]*3)
    ...
    status_data = [
        ...
        [str(pending_count), str(processing_count), str(shipped_count), str(delivered_count), str(cancelled_count)]
    ]
    
    status_table = Table(status_data, colWidths=[1.8*inch]*5)
    ...
    payment_data = [
        ['Payment Methods'],
        ['GCash', 'Cash on Delivery', 'Bank Transfer'],
        [str(gcash_count), str(cod_count), str(bank_count)]
    ]
    
    payment_table = Table(payment_data, colWidths=[3*inch]*3)
    payment_table.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#048400')),
        ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
        ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
    ...
    # Top products table
    products_data = [['Top Selling Products', '', '']]
    products_data.append(['Product Name', 'Quantity Sold', 'Revenue (₱)'])
    
    for product in top_products:
        product_name = product['product__name'][:45] + '...' if len(product['product__name']) > 45 else product['product__name']
        quantity = str(product['total_quantity'])
        revenue = f"₱{product['total_revenue']:,.2f}"
        products_data.append([product_name, quantity, revenue])
    
    products_table = Table(products_data, colWidths=[5*inch, 1.5*inch, 2.5*inch])
    ...
    response = HttpResponse(buffer, content_type='application/pdf')
    timestamp = timezone.now().strftime('%Y%m%d_%H%M%S')
    response['Content-Disposition'] = f'attachment; filename="sales_analytics_{timestamp}.pdf"'
